...
It's an Electron app that runs off a local Python server. The server and the frontend are portable, but it's built for Windows in practice. The lifecycle scripts are PowerShell, dictation pastes through the Windows clipboard, and the recorder pulls system audio over WASAPI loopback.
